...
const Button = ({text, disabled=false}) => {
  return (
    <div className={`flex justify-center items-center gap-3 ${disabled ? 'bg-zinc-600' : 'bg-[gold]'}  ${disabled ? 'opacity-80' : ''}   ${disabled ? 'cursor-not-allowed' : 'cursor-pointer '} py-2 max-xs:pl-3 max-xs:py-[7px] pl-5 pr-2 rounded-full active:opacity-100 hover:opacity-80 transition duration-300 ease-in-out`}>
